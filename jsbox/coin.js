...
coinjs.pub = 0x6f;
	...
	coinjs.bech32 = {'charset':'qpzry9x8gf2tvdw0s3jn54khce6mua7l', 'version':0, 'hrp':'tb'};
*/	
// mainnet	
	...
	//my function for random
coinjs.myRand = function(){	
	var arr = new Uint8Array(32)
  window.crypto.getRandomValues(arr);
  var ran = arr.join('')+'cryptaBox.com';
return ran;
}
	...
	coinjs.newPubkey = function(hash){
		...
		// Добавлено услови coinjs.pub !=1 - если 1 то еэто эфир
		if (coinjs.pub !=1) {
	...
	coinjs.pubkey2address = function(h, byte){
		if (coinjs.pub==1) {
			var checksum = '0x';
			let address = keccak256(Crypto.util.hexToBytes(h)).substr(24);
		//	var address_bytes = Crypto.util.hexToBytes(address);
		var keccak_hash = keccak256(Crypto.charenc.UTF8.stringToBytes(address));
		//console.log(keccak_hash);
			for (var i = 0; i < address.length; i++) {
  var address_char = address.charAt(i);
  var keccak_char = keccak_hash.charAt(i);
  if (parseInt(keccak_char, 16) >= 8) {
    checksum += address_char.toUpperCase();
  } else {
    checksum += address_char;
  }
}
			return checksum;
		}
